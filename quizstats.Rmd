...
clean_data <- quiz_data %>%
  ...
  mutate(team_weighted_score = scaled_score * max(team_size) / team_size,
  ...
  left_join(
    filter(., n_quizzes >= min_quizzes) %>%
      summarise(overall_rank = mean(team_weighted_score),
...
clean_data %>%
  ...
  fmt_number(columns = vars("Mean score (overall)", 
